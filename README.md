...
- `apps/in-sample`: minimal **`.in` v0.2** module (struct + helper **`fn`** + **`fn main`** with bounded body statements); run **`./scripts/check-in-lang-sample.sh`** (**`in build --parser in --path …`**) — no **`swiftc`**.
...
in agent --path apps/icore-sample/min.icore --parser icore
