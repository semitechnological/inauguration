...
| Backend report | Report owned executable backend support without delegating to language toolchains. | `in backend --path <file> --target bytecode --json` compiles supported Core IR paths to bytecode facts. `--target native` reports `native-backend-not-implemented`. |
