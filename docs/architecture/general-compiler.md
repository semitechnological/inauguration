...
| External compiler parity | Compare checked-in polyglot examples against installed reference toolchains where present. | `scripts/check-external-compiler-parity.sh` runs `in build` plus native compiler syntax/compile checks and reports missing tools as skips. |
