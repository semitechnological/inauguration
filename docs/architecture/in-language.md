...
- Top-level **`bind name as alias;`** — accepted as an explicit semantic package binding fact. The binding resolves only when the matching `use name;` import resolves against the nearest `inauguration.package`. Reports expose the bound alias as `symbol:binding:<alias>` and suppress `INPKG002` for direct calls to that alias. It does not install dependencies, load extensions, or execute package code.
...
- The bytecode VM executes the bounded self-hosting subset from **`std.io`**, **`std.fs`**, **`std.json`**, **`std.env`**, and **`std.path`** plus compiler-oriented core string helpers: `print`, `read_file`, `write_file`, `json_stringify`, `env_get`, `env_has`, `path_join`, `path_dirname`, `path_basename`, `path_extname`, `str_trim`, `str_split_lines`, `str_split_spaces`, `str_tokenize_expr`, `str_to_int`, `str_starts_with`, `str_index_of`, `str_slice`, and `str_is_int`. `std.http`, `std.process`, package installation, plugin execution, `json_parse`, `env_set`, and `path_normalize` remain declaration/contract-only.
- `apps/in-compiler-bootstrap` is the current executable self-hosting fixture: it reads a tiny expression source, skips blank lines and `#` comments, tokenizes whitespace-tolerant integer `let` assignments with identifiers, precedence, and parenthesized grouping, keeps a bounded string-backed symbol table, emits `.icore` function declarations with binary/call expression trees, writes `/tmp/in-compiler-bootstrap-generated.icore`, writes `/tmp/in-compiler-bootstrap-diagnostic.json` for unsupported source, and validates through bytecode execution.
