# `.in` language (v0.2 today, v0.3 orchestration contract)
...
- Top-level **`enable name;`** — parsed as an orchestration extension fact. It does not load plugin/runtime code by itself.
...
- Top-level **`distributed fn name(...)`** — parsed as a distributed-function orchestration fact. It does not provide a distributed runtime, worker scheduler, retries, or persistence.
- Top-level **`parallel { ... }`** — counted as an orchestration region. It does not provide parallel runtime execution semantics yet.
...
## v0.3 orchestration surfaces

The v0.3 contract is visibility before execution. `.in` should feed four strict public surfaces:
