...
- Standard imports **`std.io`**, **`std.fs`**, and **`std.http`** synthesize bounded extern-style Core IR declarations for `print`, `read_file`, `write_file`, and `http_get`, with capability requirements checked by `in agent`.
