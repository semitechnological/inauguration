...
- The bytecode VM executes the bounded self-hosting subset from **`std.io`**, **`std.fs`**, **`std.env`**, and **`std.path`**: `print`, `read_file`, `write_file`, `env_get`, `env_has`, `path_join`, `path_dirname`, `path_basename`, and `path_extname`. `std.http`, `std.process`, package installation, plugin execution, `env_set`, and `path_normalize` remain declaration/contract-only.
