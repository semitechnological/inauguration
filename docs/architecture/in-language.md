...
- `apps/in-compiler-bootstrap` is the current executable self-hosting fixture: it reads a tiny expression source, tokenizes no-space `let` assignments, emits `.icore` function declarations with binary/call expression trees, writes `/tmp/in-compiler-bootstrap-generated.icore`, and validates through bytecode execution.
