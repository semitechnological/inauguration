...
`in-cli/src/parser_registry.rs` resolves which **front** runs before the hybrid SIL pipeline. **Full** Core IR fronts: **`.in`**, **`.icore` (JSON)**. Dedicated language fronts now include **Rust** (`compiler::rust_front`), **Go** (`compiler::go_front`), **V** (`compiler::v_front`), and **OCaml** (`compiler::ocaml_front`) with real function/struct or top-level function lowering plus bounded body subsets (not full language semantics yet). Other tracked extensions use **`compiler::tree_front`** (**Tree-sitter** grammars → `UnifiedModule`). Java/Groovy, JavaScript/TypeScript, Kotlin, C#, Python, Ruby, Zig, and C-family functions have bounded scalar body extraction where wired. Parser ids without a wired grammar return an error that directs callers to `.icore` — see [general-compiler.md](general-compiler.md).
