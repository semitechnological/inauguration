...
| 2 | Lowers a bounded statement/expression subset into Core IR. | `.in` (including agent-facing package/module/import facts, capabilities, and extern function binding declarations), `icore` v2, `rust`, `go`, `v`, `ocaml`, `java`, `groovy`, `javascript`, `typescript`, `kotlin`, `csharp`, `python`, `ruby`, `zig`, `dart`; C / C++ / Objective-C++ functions support bounded scalar bodies and C-family runtime/ABI remains unowned. |
