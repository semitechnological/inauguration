...
- **Body** may be omitted or may appear in a brace-balanced top-level function block. The subset accepts bounded body statements: `let name = expr`, `let name: Type = expr`, `name = expr`, expression calls, `return`, `return expr`, `if condition { ... } else { ... }`, and `while condition { ... }`.
...
- `while` conditions inside bounded bodies must infer to **`Bool`** (`E_WHILE_COND_TYPE`).
...
- **`enum`**, **`class`**, **`actor`**, **`protocol`**, **`extension`**, **`typealias`**, top-level **`var`/`let`**, computed properties, **initializers**, **deinit**, accessors.
...
- Nested declarations, closures, member access outside declared struct fields, subscripts, operators outside the bounded expression set, and Swift-specific statement semantics.
